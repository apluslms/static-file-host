...
def files_to_update_1(manifest_client, manifest_srv, client_basedir):
    """
    :param manifest_client: a nested dict dict[file] = {'size': , 'mtime':, "checksum": } in the client-side
    :param manifest_srv: a nested dict dict[file] = {'size': , 'mtime':, "checksum": } in the server side
    ...
    file_update = [f for f in file_inter if not math.isclose(manifest_client[f]["mtime"], manifest_srv[f]["mtime"])]

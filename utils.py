...
def upload_files(files_and_sizes, basedir, upload_url, index_mtime):
    ...
    Arguments:
        ...
        index_mtime {float} -- modification time of the index file (possibly no need to check)
    """
    ...
    for f in small_files:
        print(f[0],os.path.getctime(f[0]))
    # print(small_files)
    ...
    if small_files:
        ...
        compress_files_upload2(small_files, basedir, 4 * 1024 * 1024, upload_url, headers, data)


def compress_files_upload2(file_list, basedir, buff_size_threshold, upload_url, headers, data):
    ...
    """
    print("compress_files_upload2")
    last_file = file_list[-1][0]
    # Generate the buffer of the compression file that contains the files in the file_list
    buffer = tar_filelist_buffer2(file_list, basedir)
    ...
    if pos <= buff_size_threshold or len(file_list) == 1:  # post the buffer
        ...
        for l in file_sublists:
            compress_files_upload2(l, basedir, buff_size_threshold, upload_url, headers, data)


def tar_filelist_buffer2(files, basedir):
...
def get_file_manifest(directory):
    ...
    :return: a dict with rel_file_name as keys and their creation time as values
    """
    manifest = dict()
